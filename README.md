...
The code is designed to import from a mounted google drive at `/content/drive/MyDrive/nuclei_labelling/` but the code is also designed to work if you upload the data and model yourself. It looks in the local directory before trying to import from google drive.

If no model is found in the local drive or in a mounted google drive, it will train a new model. Making it so that the only requirement to run the code is the `data-science-bowl-2018.zip` at `/content/`.
